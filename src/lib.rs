...
use blake2::{Blake2b, Blake2bVar, Blake2s256, Digest};
...
fn neighborhood_aggregate(graph: &DiGraph<String, ()>, node: NodeIndex, node_labels: &HashMap<NodeIndex, String>) -> String {
    let mut label_list: Vec<String> = graph.neighbors(node)
        .map(|nbr| node_labels.get(&nbr).unwrap().clone())
        .collect();
    label_list.sort();
    node_labels.get(&node).unwrap().clone() + &label_list.concat()
}

fn weisfeiler_lehman_step(graph: &DiGraph<String, ()>, labels: &HashMap<NodeIndex, String>, digest_size: usize) -> HashMap<NodeIndex, String> {
    graph.node_indices().map(|node| {
        let label = neighborhood_aggregate(graph, node, labels);
        (node, hash_label(&label, digest_size).unwrap())
    }).collect()
}

pub fn weisfeiler_lehman_graph_hash(graph: &DiGraph<String, ()>, node_attr: Option<&str>, iterations: usize, digest_size: usize) -> String {
    ...
    hash_label(&final_label, digest_size).unwrap()
}
...
mod tests {
    ...
    fn test_weisfeiler_lehman_graph_hash() {
        ...
        let hash = weisfeiler_lehman_graph_hash(&graph, None, 3, 16);
        println!("{}",hash);
    ...
    fn test_weisfeiler_lehman_graph_hash_isomorphic() {
        ...
        let hash1 = weisfeiler_lehman_graph_hash(&g1, None, 3, 16);
        let hash2 = weisfeiler_lehman_graph_hash(&g2, None, 3, 16);
    ...
    fn test_weisfeiler_lehman_graph_hash_non_isomorphic() {
        ...
        let hash1 = weisfeiler_lehman_graph_hash(&g1, Some("label"), 3, 16);
        let hash2 = weisfeiler_lehman_graph_hash(&g2, Some("label"), 3, 16);
